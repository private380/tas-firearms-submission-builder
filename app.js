...
function renderDocument(doc,targetId,forPrint=false){
 ...
 doc.sections.filter(s=>s.type==="amendment").forEach(s=>counts[s.position]++);

 const detailsHtml=Object.values(details).some(Boolean)?`
   <section class="document-details">
     <h2>About the submitter</h2>
     <dl>
       ${details.name?`<dt>Name</dt><dd>${esc(details.name)}</dd>`:""}
       ${details.town?`<dt>Location</dt><dd>${esc(details.town)}</dd>`:""}
       ${details.email?`<dt>Email</dt><dd>${esc(details.email)}</dd>`:""}
       ${details.background?`<dt>Background or interest</dt><dd>${esc(details.background)}</dd>`:""}
     </dl>
   </section>`:"";

 const amendmentSections=doc.sections.filter(s=>s.type==="amendment");
 const summaryRows=amendmentSections.map(s=>`
   <tr>
     <td>${s.number}</td>
     <td>${esc(s.title)}</td>
 ...
 const summaryHtml=`
   <section class="document-summary">
     ...
     <table class="summary-table">
       <thead><tr><th>No.</th><th>Amendment</th><th>Position</th></tr></thead>
       <tbody>${summaryRows}</tbody>
     </table>
   </section>`;

 const sectionHtml=doc.sections.filter(s=>s.type!=="details").map(s=>{
   if(s.type==="amendment"){
   ...
   return `<section class="document-amendment position-neutral"><h2>${esc(s.title)}</h2><p>${esc(s.body).replace(/\n/g,"<br>")}</p></section>`;
 }).join("");

 target.innerHTML=`<div class="document-page">
   <header class="document-header">
     <div class="document-kicker">Individual consultation submission</div>
     <h1>Firearms Amendment (Miscellaneous) Bill 2026</h1>
     <p>Prepared in response to the Tasmanian Government consultation.</p>
   </header>
   <section class="document-recipient">
