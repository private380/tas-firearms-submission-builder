...
function renderDocument(doc,targetId,forPrint=false){
 ...
 const sectionHtml=doc.sections.filter(s=>s.type!=="details").map(s=>{
   if(s.type==="amendment"){
     ...
     return `<section class="document-amendment">
       <div class="document-amendment-number">Amendment ${s.number}</div>
       <h2>${esc(s.title)}</h2>
       <div class="document-position"><span>Position</span><strong>${esc(s.position)}</strong></div>
   ...
   return `<section class="document-amendment"><h2>${esc(s.title)}</h2><p>${esc(s.body).replace(/\n/g,"<br>")}</p></section>`;
 }).join("");
