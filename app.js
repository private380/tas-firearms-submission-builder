...
function buildAnonymousAnalyticsPayload(){
 const positions=state.answers.map((answer,index)=>({
   amendment:index+1,
   position:answer.position||"No comment"
 }));

 return {
   positions,
   furtherSuggestions:[...(state.selectedFurtherSuggestions||[])],
   closingSuggestions:[...(state.selectedClosingSuggestions||[])]
 };
}

function createPdf(){
 const shouldRecordGeneration=!state.countedCurrentSubmission;
 if(shouldRecordGeneration && typeof window.recordGlobalSubmission==="function"){
   const analyticsPayload=buildAnonymousAnalyticsPayload();
   window.recordGlobalSubmission(analyticsPayload)
     .then(recorded=>{
       if(recorded){
