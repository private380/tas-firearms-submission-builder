...
let state={current:0,answers:Array.from({length:21},()=>({position:"",reasons:[],otherReason:"",alternatives:[],otherAlternative:"",comments:""})),details:{},selectedFurtherSuggestions:[],selectedClosingSuggestions:[],suggestions:"",closing:""};
...
const FURTHER_SUGGESTIONS=[
 ...
 "Some proposals may address circumstances highlighted by the Bondi attack, but the Bill does not adequately address outdated or ineffective storage requirements.",
 ...
 "Separate broadly supported administrative and criminal-law reforms from contentious firearm reclassifications.",
...
function renderFinalSuggestions(){
 ...
 $("furtherSuggestionOptions").querySelectorAll("input").forEach(c=>c.onchange=()=>{
   state.selectedFurtherSuggestions=[...$("furtherSuggestionOptions").querySelectorAll("input:checked")].map(q=>q.parentElement.querySelector("span").textContent);
   save();
 });
 ...
 $("closingSuggestionOptions").querySelectorAll("input").forEach(c=>c.onchange=()=>{
   state.selectedClosingSuggestions=[...$("closingSuggestionOptions").querySelectorAll("input:checked")].map(q=>q.parentElement.querySelector("span").textContent);
   save();
 });
}
...
["name","town","email","background","suggestions","closing"].forEach(id=>$(id).addEventListener("input",save));
