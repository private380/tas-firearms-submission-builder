...
function renderPrintReport(generated,data){
  const rows=Array.from({length:21},(_,idx)=>amendmentRow(idx+1,data)).filter(x=>x.total);

  $("printStats").innerHTML=[
    ["PDFs generated",fmt(generated)],
    ["Anonymous result records",fmt(data.recordedSubmissions||0)],
    ["Amendments surveyed","21"]
  ].map(([label,value])=>`<div class="print-stat"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`).join("");
  ...
  $("printFindings").innerHTML=[
    [`${majorityOppose} of 21`,`amendments recorded more than 50% opposition.`],
    [`${majoritySupport} of 21`,`amendments recorded more than 50% support.`],
    [strongestOppose?`Amendment ${strongestOppose.i} · ${pctText(strongestOppose.counts.oppose,strongestOppose.total)}`:"—","highest recorded opposition."],
    [strongestSupport?`Amendment ${strongestSupport.i} · ${pctText(strongestSupport.counts.support,strongestSupport.total)}`:"—","highest recorded support."]
  ].map(([a,b])=>`<div class="print-finding"><strong>${esc(a)}</strong><span>${esc(b)}</span></div>`).join("");

  $("printAmendments").innerHTML=rows.map(x=>{
    const bars=Object.entries(x.counts).map(([k,v])=>`<span class="${posClass(k)}" style="width:${pct(v,x.total)}%"></span>`).join("");
    const body=Object.entries(x.counts).map(([k,v])=>`<tr><td>${posLabel(k)}</td><td>${fmt(v)}</td><td>${pctText(v,x.total)}</td></tr>`).join("");
    return `<article class="print-amendment">
      <div class="print-amendment-head">
        <div>
          <div class="print-amendment-number">Amendment ${x.i}</div>
          <div class="print-amendment-title">${esc(x.title)}</div>
        </div>
        <div class="print-lead"><strong>${x.leader?`${posLabel(x.leader[0])} ${pctText(x.leader[1],x.total)}`:"No data"}</strong><span>${fmt(x.total)} responses</span></div>
      </div>
      <div class="print-bar">${bars}</div>
  ...
  function rankedHtml(obj,limit=15){
    const arr=Object.values(obj||{}).filter(x=>x&&x.label)
      .map(x=>({label:String(x.label),count:+(x.count||0)}))
      .sort((a,b)=>b.count-a.count).slice(0,limit);
    return arr.length?arr.map((x,i)=>`<div class="print-rank"><div class="n">${i+1}</div><div>${esc(x.label)}</div><div class="count">${fmt(x.count)}</div></div>`).join("")
      :"<p>No aggregate selections were recorded.</p>";
  }

  $("printFurther").innerHTML=rankedHtml(data.furtherSuggestions);
  $("printClosing").innerHTML=rankedHtml(data.closingSuggestions);
}
...
async function loadResults(){
  try{
    ...
    renderPrintReport(generated,data);
  }catch(err){
    console.error(err);
