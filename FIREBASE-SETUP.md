...
Version 6.1 writes running totals under `communityResults`:

- Support, Oppose, Unsure and No comment counts for each amendment.
- Counts for each selected Further suggestion.
- Counts for each selected Closing suggestion.

It does not transmit or store names, email addresses, locations, background details, free-text comments or PDFs.

Publish the updated `database.rules.json` before deploying Version 6.1.
